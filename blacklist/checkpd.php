<?php 
    $telToAdd = $_POST["telPD"];
    $cause = $_POST["causePD"];
    $fillerName = $_POST["fillerNamePD"];

?>
<!-- ทำการเช็คว่าเบอร์นี้เคยโดนมาหรือยัง -->

<?php
    require('connect.php');
    $sql = "SELECT * FROM blacklist_tspd;";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) { 
    while($row = mysqli_fetch_assoc($result)) {
        if ($telToAdd == $row['tel']){
            if ($row['status'] == 1) {
                $addTel = mysqli_query($conn, "UPDATE blacklist_tspd SET status=2 WHERE tel=$telToAdd");
                break;
            } elseif ($row['status'] == 2) {
                    $addTel = mysqli_query($conn, "UPDATE blacklist_tspd SET status=3 WHERE tel=$telToAdd");
                    break;
        }
            break;
        } else {
            $addTel = mysqli_query($conn, "INSERT INTO blacklist_tspd (tel) VALUES ('$telToAdd')");
        }
    }} 
?>
